CREATE PROCEDURE [OTACoins].[AddTransaction]
    @instance INT,
    @transactionHash BINARY(32),
    @mutationHash BINARY(32),
    @rawData VARBINARY(MAX),
    @records [OTACoins].[RecordMutationTable] READONLY
AS
    SET XACT_ABORT ON;
    SET NOCOUNT ON;
    ...
    LEFT OUTER JOIN [OTACoins].[Records] ON (UpdatedRecords.[Key] = Records.[Key] AND Records.[Instance] = @instance)
    WHERE UpdatedRecords.[Version] <> ISNULL(Records.[Version], CAST(0x AS VARBINARY(32)));

    IF @@Rowcount > 0
        RETURN;

    MERGE [OTACoins].[Records] AS Target
    ...
    INSERT INTO [OTACoins].[Transactions]
    ...
    INSERT INTO [OTACoins].[RecordMutations]
